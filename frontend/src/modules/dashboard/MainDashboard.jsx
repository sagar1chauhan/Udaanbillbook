      ...
      <PageHeader
        ...
        actions={
          <div className="flex w-full items-center gap-2">
            ...
            <Button
              variant="outline"
              className="rounded-xl px-2 text-xs sm:text-sm sm:px-4 shrink-0 border-amber-200 bg-amber-50 text-amber-700 hover:bg-amber-100 hover:text-amber-800"
              onClick={handleTestNotification}
              disabled={sendingTest}
              title="Send Test Push Notification"
            >
              {sendingTest ? (
                <Loader2 className="mr-1 h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0 animate-spin" />
              ) : (
                <Bell className="mr-1 h-3.5 w-3.5 sm:h-4 sm:w-4 shrink-0" />
              )}
              <span className="truncate">{sendingTest ? 'Sending...' : 'Test 🔔'}</span>
            </Button>
